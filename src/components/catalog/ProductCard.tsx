  ...
  if (viewMode === 'list') {
    return (
      <div
        ...
        className="group cursor-pointer bg-white rounded-2xl border border-slate-200 p-4 hover:border-slate-400 hover:shadow-md transition-all duration-200 flex flex-col sm:flex-row items-center gap-5"
      >
        <Link
          href={`/catalogo/${product.slug}`}
          onClick={(e) => e.stopPropagation()}
          className="relative w-full sm:w-36 h-36 rounded-xl overflow-hidden bg-slate-50 shrink-0 block"
        >
          {product.images?.[0] ? (
            <Image
        ...
        <div className="flex-1 w-full min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <Link
              href={`/catalogo/${product.slug}`}
              onClick={(e) => e.stopPropagation()}
              className="text-[11px] font-bold text-slate-800 hover:text-red-700 inline-flex items-center gap-1 transition-colors"
            >
    ...
    >
      <div>
        ...
        <Link
          href={`/catalogo/${product.slug}`}
          onClick={(e) => e.stopPropagation()}
          className="relative w-full aspect-square bg-slate-50 overflow-hidden border-b border-slate-100 block"
        >
          {product.images?.[0] ? (
            <Image
              src={product.images[0]}
      ...
      <div className="p-4 sm:p-5 pt-0 grid grid-cols-2 gap-2">
        {/* Primary Action: Add / Quote Button */}
        <button
          onClick={handleAdd}
